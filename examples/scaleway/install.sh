...
cert_manager() {
  ...
  echo "Installing Scaleway Cert Manager"
  rm -rf tmp/cert-manager-webhook-scaleway
  git clone https://github.com/scaleway/cert-manager-webhook-scaleway.git tmp/cert-manager-webhook-scaleway
  helm upgrade \
    --atomic \
    --cleanup-on-fail \
    --install \
    ...
    --wait \
    scaleway-webhook tmp/cert-manager-webhook-scaleway/deploy/scaleway-webhook
  ...
  yq e \
    "(.stringData.SCW_ACCESS_KEY=\"${SCW_ACCESS_KEY}\" | .stringData.SCW_SECRET_KEY=\"${SCW_SECRET_KEY}\")" "$(get_file kubernetes/scaleway-secret.yaml)" | kubectl apply -f -
